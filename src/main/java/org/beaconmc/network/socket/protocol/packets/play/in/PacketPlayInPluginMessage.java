...
@NoArgsConstructor
@AllArgsConstructor
public class PacketPlayInPluginMessage extends PacketPlay implements PacketIn {
    ...
    public void read(PacketSerializer packetSerializer, int protocolVersion) {
        this.channel = packetSerializer.readString();
        byte[] data = new byte[packetSerializer.readVarInt()];
        packetSerializer.getByteBuf().readBytes(data);
        this.data = data;
    }
