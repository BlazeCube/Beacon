...
@NoArgsConstructor
@AllArgsConstructor
public class PacketLoginOutSuccess extends PacketLogin implements PacketOut, AsyncPacket {
    ...
    public PacketLoginOutSuccess(PlayerProfile playerProfile){
        this.username = playerProfile.getUsername();
        this.uuid = playerProfile.getUuid().toString();
    }

    @Override
    public void write(PacketSerializer packetSerializer, int protocolVersion) {
        packetSerializer.writeString(this.uuid);
